defmodule Spec.MismatchTest do
  ...
  describe "to_string" do
    ...
    test "on predicate failure" do
      ...
      Spec.Mismatch: `:a` does not satisfy predicate `is_number()`
      """ |> rstrip
    end
    ...
    test "on nested predicate failure on list item" do
      {:error, mismatch = %Mismatch{}} = conform([is_number()], ["hello"])
      assert to_string(mismatch) == """
      Spec.Mismatch: `"hello"` does not satisfy predicate `is_number()`
    ...
    test "on invalid keyword values" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: Inside `[a: 1]`, one failure:
    ...
    test "on invalid or" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `2` does not match any alternative `is_atom() or is_binary() or is_list()`
      """ |> rstrip
    end
    ...
    test "on invalid and" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `2` does not match all alternatives `is_number() and is_binary() and is_list()`
    ...
    test "on invalid tuple" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `[1]` is not a tuple
      """ |> rstrip
    end
    ...
    test "on invalid tuple size" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `{1, 2}` does not have length 1
      """ |> rstrip
    end
    ...
    test "on missing required key" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `%{b: 1}` does not have key `[:a]`
      """ |> rstrip
    end
    ...
    test "on invalid keyword keys with or operator" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `[a: 1]` does not have any of keys `[:d, :c]`
      """ |> rstrip
    end
    ...
    test "on invalid one_or_more" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `[]` does not have min length of 1
      """ |> rstrip
    end
    ...
    test "on invalid one_or_more match" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `"a"` does not satisfy predicate `is_integer()`
      """ |> rstrip
    end
    ...
    test "on invalid zero_or_one size" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `[1, 2]` does not have max length of 1
      """ |> rstrip
    end
    ...
    test "on many with stream max" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `1..10` does not have max length of 3
      """ |> rstrip
    end
    ...
    test "on many with stream min" do
      ...
      assert to_string(mismatch) == """
      Spec.Mismatch: `1..3` does not have min length of 10
      """ |> rstrip
    end
  end
