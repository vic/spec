defmodule Spec.Conformer do
  ...
  defp quoted_expr(quoted = {:::, _, [{name, _, x}, arg]}) when is_atom(name) and is_atom(x) do
    ...
    expr = quote do
      fn value ->
        {unquote(name), value} |> Spec.Conformer.pipe(unquote(conformer))
      end.()
    end
    quoted_fn(expr, quoted)
  end
  ...
  defp quoted_expr(quoted = {:and, _, [a, b]}) do
    ...
    expr = quote do
      fn value ->
        Spec.Conformer.pipe(value, unquote(a))
        |> case do
          {:ok, conformed} ->
            Spec.Conformer.pipe(conformed, unquote(b))
          {:error, e} -> 
    ...
    expr = quote do
      fn value ->
        ...
        |> case do
          ...
          {:error, _} ->
            Spec.Conformer.pipe(value, unquote(b))
            |> case do
                {:ok, _} = ok -> ok
                {:error, _} ->
                   Mismatch.error(subject: value,
